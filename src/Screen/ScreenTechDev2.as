package Screen 
{
	 ...
	 * @author vikachew
	 */
	public class ScreenTechDev2 extends SCREEN_TECHDEV2
	{
		...
		public function ScreenTechDev2(passedClass:Main, passedPP:int) 
		{
		...
		private function getDataSuccess(event:SQLEvent):void {
			...
			// Outline
			format.size = 21;
			var txtLabelOutline:TextField = new TextField();
			txtLabelOutline.text = "OUTLINE"
			format.color = 0xD4D4D4; 
			txtLabelOutline.setTextFormat(format); 
			txtLabelOutline.embedFonts = true;
			txtLabelOutline.width = 300;
			txtLabelOutline.selectable = false;
			
			var txtOutline:TextField = new TextField();
			if (mainClass.bahasa) txtOutline.text = row.outlineId;
			else {
				if (row.outlineEn == null || row.outlineEn == "" || row.outlineEn == "null") txtOutline.text = row.outlineId;
				else txtOutline.text = row.outlineEn;
			}
			format.size = 16;
			format.leading = 0;
			format.align = "left";
			format.color = 0xFFFFFF; 
			txtOutline.setTextFormat(format); 
			txtOutline.embedFonts = true;
			
			txtOutline.width = 400;
			txtOutline.autoSize = "left";
			txtOutline.multiline = true;
			txtOutline.wordWrap = true;
			txtOutline.selectable = false;
			...
			if (mainClass.bahasa) txtOut.text = row.outcomeId;
			else {
				if (row.outcomeEn == null || row.outcomeEn == "" || row.outcomeEn == "null") txtOut.text = row.outcomeId;
				else  txtOut.text = row.outcomeEn;
			}
			format.color = 0xFFFFFF; 
			format.align = "left";
			format.size = 16;
			...
			container.scrollRect = new Rectangle(0, 0, 500, 700 + txtOutline.height + txtOut.height);
			
			var square:Shape = new Shape(); 
			square.graphics.lineStyle(1, 0x000000, 0.0); 
			square.graphics.beginFill(0xff0000, 0.0); 
			square.graphics.drawRect(0, 0, 500, 700 + txtOutline.height + txtOut.height); 
			...
			// SEMEN, AREA, PERIOD, SCOPE
			var txtHeaderSemen:TextField = new TextField();
			...
			format.leading = -7;
			format.indent = 0;
			if (row.idsemen == 1) txtHeaderSemen.text = "SEMEN GRESIK";
			else if (row.idsemen == 2) txtHeaderSemen.text = "SEMEN PADANG";
			else if (row.idsemen == 3) txtHeaderSemen.text = "SEMEN TONASA";
			else if (row.idsemen == 4) txtHeaderSemen.text = "SEMEN THANGLONG";
			else if (row.idsemen == 5) txtHeaderSemen.text = "SEMEN INDONESIA";
			...
			txtHeaderSemen.y = txtJudul.y + txtJudul.height + 20;
			txtHeaderSemen.x = 90;
			
			txtHeaderArea.y = txtHeaderSemen.y + 30;
			txtHeaderArea.x = txtHeaderSemen.x;
			txtArea.y = txtHeaderArea.y;
			txtArea.x = 190;
			
			txtHeaderPer.y = txtArea.y + txtArea.height + 5;
			txtHeaderPer.x = txtHeaderSemen.x;
			txtPer.y = txtHeaderPer.y;
			txtPer.x = txtArea.x;
			
			txtHeaderScope.y = txtHeaderPer.y + 30;
			txtHeaderScope.x = txtHeaderSemen.x;
			txtScope.y = txtHeaderScope.y;
			txtScope.x = txtArea.x;
			...
			var sql:String = "select * from tdimages where idtd = " + selectedPP;
			...
			// Artikel
			txtLabelOutline.y = 0;
			txtOutline.y = txtLabelOutline.y + 40;
			txtLabelOut.y = txtOutline.y + txtOutline.height + 50;
			...
			addChild(txtScope);
			//addChild(imageMC);
			container.addChild(txtLabelOutline);
			container.addChild(txtOutline);
