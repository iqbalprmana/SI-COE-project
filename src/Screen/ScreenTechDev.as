package Screen 
{
	 ...
	 * @author vikachew
	 */
	public class ScreenTechDev extends SCREEN_TECHDEV
	{
		...
		public function ScreenTechDev(passedClass:Main, passedSemen:int) 
		{
		...
		private function getData():void {
			...
			var sql:String = "select * from techdev where idsemen = " + selectedSemen + " order by lastEdit DESC"
			selectStmt.text = sql;
		...
		private function getDataSuccess(event:SQLEvent):void {
			...
			container = new MovieClip();
			container.scrollRect = new Rectangle(0, 0, 1000, numResults * 360);
			
			var square:Shape = new Shape(); 
			square.graphics.lineStyle(1, 0x000000, 0.0); 
			square.graphics.beginFill(0xff0000, 0.0); 
			square.graphics.drawRect(0, 0, 1000, numResults * 360); 
			...
			{ 
				...
				txtPeriod.y = 10 + txtJudul.y + txtJudul.height;
				txtPeriod.x = 400;
				txtJudul.width = 1000;
				txtPeriod.selectable = false;
				
				container.addChild(txtPeriod);
				...
				txtLabelPeriod.y = txtPeriod.y;
				txtLabelPeriod.x = 300;
				txtLabelPeriod.selectable = false;
				
				container.addChild(txtLabelPeriod);
		...
		private function onImageLoaded(e:Event):void {			
			...
			targetLoader.width = 440;
			targetLoader.height = 325;
		}
		...
		private function inAnim8():void {
			TweenLite.to(mcTDRed, 0.5, { alpha:1, scaleY:1, scaleX:1, delay: 0.3 } );
			setTextColor();
			init();
